...
off-hours fair value oracle for tokenized equities on robinhood chain.

when nyse and nasdaq are closed, fletch publishes a fair value estimate per
...
packages/contracts FletchCommits.sol + foundry deploy script.
...
proxy sources (24/7) -/         |
                        ...
                        engine (fair value)
                                |
                                v
                    merkle root ---> FletchCommits (robinhood chain)
```
